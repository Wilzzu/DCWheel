...
const RemoveAllButton = ({ players, setPlayers }) => {
	return (
		<div className="group absolute right-0 flex justify-center">
			...
			{/* Tooltip */}
			{players.length > 0 && (
				<span className="absolute z-10 pointer-events-none text-[2px] 2xl:text-[5px] font-medium group-hover:text-[0.65rem] 2xl:group-hover:text-xs rounded-md text-nowrap -top-2 p-1 2xl:px-2 2xl:py-[0.36rem] bg-red-800 border border-red-600 opacity-0 group-hover:-top-9 group-hover:opacity-100 duration-200">
					Remove all
				</span>
			)}
		</div>
